...
# 6. TTS через curl (обход проблем с библиотеками в Python 3.14)
def yandex_tts(text: str, voice: str, lang: str, filename: str) -> int:
    ...
    if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
        ...
        return len(text) * 60
    raise Exception("TTS failed or empty file")

def split_sentences(text: str) -> list:
    return [s.strip() for s in re.split(r'(?<=[.!?])\s+', text) if s.strip()]

# 7. Эндпоинт генерации
...
def generate_lesson(req: LessonRequest):
    ...
    for i, orig in enumerate(sentences):
        ...
        explanation = "💡 Обрати внимание на структуру предложения." if req.include_explanations else None
        
        en_file, ru_file = f"en_{i}.mp3", f"ru_{i}.mp3"
        ...
        manifest["sentences"].append({
            "id": i, "original": orig, "translation": trans, "explanation": explanation,
            "audio": {"en": f"/output/{lesson_id}/{en_file}", "ru": f"/output/{lesson_id}/{ru_file}"}
        })
